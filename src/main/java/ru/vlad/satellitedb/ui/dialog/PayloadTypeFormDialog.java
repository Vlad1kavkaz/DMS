...
public class PayloadTypeFormDialog {

    private final Dialog<PayloadType> dialog = new Dialog<>();
    ...
    public PayloadTypeFormDialog(PayloadType type) {
        this.type = type != null ? copyType(type) : new PayloadType();

        dialog.setTitle(type == null ? "Добавление типа полезной нагрузки" : "Редактирование типа полезной нагрузки");
    ...
    public Optional<PayloadType> showAndWaitForResult() {
    ...
    private PayloadType buildTypeFromForm() {
        type.setType(trimToNull(typeField.getText()));
        type.setDescription(trimToNull(descriptionArea.getText()));
        return type;
    }

    private PayloadType copyType(PayloadType source) {
        PayloadType copy = new PayloadType();
